...
import { useRoqClient, useFinanceChartFindFirst } from 'lib/roq';
...
function FinanceChartEditPage() {
  ...
  const { data, error, isLoading, mutate } = useFinanceChartFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: FinanceChartInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.finance_chart.update({
        data: values as RoqTypes.finance_chart,
        where: {
          id,
        },
      });
